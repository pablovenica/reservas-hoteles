...
// 🔥 OBTENER TODAS LAS RESERVAS DE UN USUARIO (ARREGLADO)
func (r *ReservationRepository) GetReservationsByUser(userID int) ([]domain.Reservation, error) {
	var reservations []domain.Reservation

	// 👉 columna REAL: id_user
	err := r.DB.Where("id_user = ?", userID).Find(&reservations).Error
	if err != nil {
		return nil, err
	}
...
func (r *ReservationRepository) IsDateAvailable(hotelID string, ingreso, salida int64) bool {
	...
	// 👉 columna REAL: id_hote
	r.DB.Model(&domain.Reservation{}).
		Where("id_hote = ?", hotelID).
		Where(`(fecha_ingreso <= ? AND fecha_salida >= ?)`, salida, ingreso).
		Count(&count)
