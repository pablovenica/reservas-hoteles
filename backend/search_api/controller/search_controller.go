...
// NUEVO: endpoint para reindexar todos los hoteles desde hotels_api hacia Solr
func ReindexHotels(c *gin.Context) {
	if err := service.ReindexAllHotels(); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Error reindexando hoteles"})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Reindex completo"})
}
